...
from src.jsonifyer.api import convert_csv

class TestCSVConversion(unittest.TestCase):
    def setUp(self):
        self.csv_path = 'test/input/sample_data.csv'
        self.output_dir = 'test/output/csv_results'
        self.expected_data = [
            {
                "Name": "John Doe",
                "Age": 30,
    ...
    def test_csv_file_exists(self):
        self.assertTrue(Path(self.csv_path).exists(), "CSV file should exist")
    ...
    def test_csv_conversion_with_fields(self):
        result = convert_csv(
            file_path=self.csv_path,
            output_path=self.output_dir,
            fields=["Name", "Age"],
    ...
    def test_csv_conversion_with_invalid_file(self):

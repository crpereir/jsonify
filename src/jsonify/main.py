...
from jsonify.converter.python_converter import parse_xml_to_json, check_null_fields
from jsonify.converter.xslt_converter import apply_xslt_to_xml
from jsonify.converter.csv_converter import convert_file_to_json
from jsonify.config_loader import ConfigLoader
import datetime
import argparse
...
def process_file_types(config_loader):
    ...
    file_counters = {file_type: _init_file_counter() for file_type in file_types}
    ...
    for file_type in file_types:
        _process_file_type(
            ...
            missing_fields_log, file_counters, total_counters, 
            unconverted_files
        )
...
def _process_file_type(
    ...
    removed_count = clean_repeated_items(processed_names_file, output_folder, file_type, log_file)
    ...
    for file in os.listdir(output_folder):
        ...
        try:
            ...
            if item_name and (item_name in processed_names):
                ...
                save_processed_names(processed_names_file, item_name)
...
def save_processed_names(file_path, item_name):
    if not item_name:
        return
...
def parse_cli_args():
    parser = argparse.ArgumentParser(
        description="Convert files to JSON format using configurable methods."
    )
    parser.add_argument('--input-dir', type=str, help='Input directory with files to convert')
    ...
    parser.add_argument('--conversion-method', choices=['python', 'xslt'], help='Conversion method for XML files')
    parser.add_argument('--file-types', type=str, help='Comma-separated list of file types to convert (e.g., xml,csv)')
...
def main():
    ...
    config_loader = ConfigLoader(config_path=args.config) if args.config else ConfigLoader()

    if args.input_dir:
        config_loader.base_input_folder = args.input_dir
    if args.output_dir:
        config_loader.base_output_folder = args.output_dir
    if args.conversion_method:
        config_loader.conversion_method = args.conversion_method
    if args.file_types:
        config_loader.file_types = [ft.strip() for ft in args.file_types.split(',')]
